...
namespace path_guiding
{
    ...
    struct quad_tree_group
    {
        ...
        quad_tree_node& getNode(int id)
        {
            if (id >= nodes.size())
            {
                printf("invalid quad tree query: %d/%zu", id, nodes.size());
                return nodes[0];
            }
            return nodes[id];
        }
    ...
    struct Spatio_tree
    {
        ...
        Spatio_tree_node& getNode(int index)
        {
            if (index >= nodes.size())
            {
                printf("invalid spatio tree query: %d/%zu", index, nodes.size());
            }
            return nodes[index];
        }
    ...
    struct SD_PGTrainer
    {
        ...
        bool build_tree(int div_limit = 24000 * 2, int path_num = -1)
        {
            ...
            printf("train %d complete, s_tree now has %zu nodes, and d_tree now has %zu nodes\n", path_k, s_tree.nodes.size(), q_tree_group.nodes.size());
