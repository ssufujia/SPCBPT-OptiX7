...
namespace classTree
{
    ...
    __host__ RT_FUNCTION int tree_index(tree_node* root, float3 position,float3 normal = make_float3(0.0f),float3 direction = make_float3(0.0f))
    {
        int node_id = 0;
        int l_id = node_id;
    ...
    __host__ RT_FUNCTION int tree_index_node_id(tree_node* root, float3 position, float3 normal = make_float3(0.0f), float3 direction = make_float3(0.0f))
    {
        int node_id = 0;
        while (root[node_id].leaf == false)
        {
