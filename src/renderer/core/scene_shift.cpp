...
static std::map<int, cudaTextureObject_t> sampler_remap;
void Material_shift(Scene& Src, sutil::Scene& Dst)
{
    for (int i = 0; i < Src.texture_map.size(); i++)
    {
        ...
        int image_id = static_cast<int>(Dst.ImagesSize()) - 1;
    ...
    for (int i = 0; i < Src.materials.size(); i++)
    {
        ...
        materialID_remap[i] = static_cast<int>(Dst.MaterialsSize());
        Dst.addMaterial(mtl);
    }
...
//        light.type = SL.lightType == LightType::DIRECTION? Light::Type::DIRECTIONAL 
        printf("light source albedo tex load %d %llu\n", i, (unsigned long long)sampler_remap[SL.albedoID]);
