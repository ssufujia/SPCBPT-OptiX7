	...
	struct PGParams
	{
		...
		RT_FUNCTION __host__ void checkUV(int u, int v) {
			if (u >= GRID_SIZE || v >= GRID_SIZE) {
				printf("Error in PGParams -- u,v should be smaller than GRID_SIZE\nu: %d\t\tv: %d\t\tgrid: %d\n", u, v, GRID_SIZE);
				exit(0);
			}
		}
		...
		RT_FUNCTION __host__ void checkGrid() {
			if (prefixSum[GRID_SIZE * GRID_SIZE - 1] < 1e-9) {
				printf("Error in PGParams -- prefixSum[%d] is %f. which is too small\n", GRID_SIZE * GRID_SIZE - 1, prefixSum[GRID_SIZE * GRID_SIZE - 1]);
				exit(0);
			}
		}

		RT_FUNCTION __host__ void train(float3* input, int size)
		{
